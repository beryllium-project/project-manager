...
| Component queues | `../analysis-workbook/outbox/pm-queue.md`, `../threat-modeler/outbox/pm-queue.md`, `../security-reviewer/outbox/pm-queue.md`, `../analysis-workbook/outbox/helium-transfer-queue.md` | Pull-only; consumed ledger-first; status edits applied as class-1 carried writes only for the `outbox/pm-queue.md` files named in `PMD-20260904-003` (the security-reviewer queue's edits are handed to the human until the responsible human extends class 1 to it; `PMD-20260906-003`); the transfer queue is read-only tracking and is never edited by the PM |
